...
- 1688 采购订单可以通过页面上传从 1688 导出的订单表格，也可以通过登录后的 `/api/wholesale/1688-orders` 接口接收，进入订单认领列表；接口每次最多接收 5 MiB、500 行，同一用户 10 分钟最多接收 5 次且同时只能执行 1 次，响应结束后释放并发名额。页面上传支持 1688 导出的 Excel 订单表格并保留 CSV 文件接收能力。订单认领页分为“已认领 / 待分类 / 认领大厅”。API 接收的 1688 订单如果能根据客户或批发订单信息确定归属，会自动进入已认领；页面上传从订单认领页按钮打开弹窗完成，不直接铺在页面顶部，订单行会先按收货人名字辅助匹配批发客户，匹配到客户的进入待分类，没有辅助匹配到客户的进入认领大厅。认领列表保留订单号、商品、客户等综合搜索，并可按收货人名字和含首尾日期的采购时间区间进一步筛选；采购日期按上海业务日期判断，启用日期条件后不显示没有有效采购时间的记录。“待分类”和“认领大厅”支持逐条勾选或全选当前筛选结果，再把整批采购订单统一归属到一个客户和批发订单；切换板块或筛选条件会清空选择，数据库保证整批同时成功或同时失败。“已认领”不提供批量改归属，管理员和业务员仍可逐条调整。认领按钮显示在固定订单号列下方，操作列保留移出等管理动作；认领后列表显示最近处理人。原始采购订单内容进入系统后不允许随意修改，误上传数据只能由管理员移出认领列表。
...
- 批发浏览器回归会拦截真实写请求，验证订单、附件、客户别名、账号合并、1688 导入、单条与批量归属、物流店铺归属、推荐、结汇发布和结汇匹配失败时表单保持打开、字段不丢失、反馈可见；1688 用例还覆盖收货人和采购日期筛选、当前结果全选、已认领板块无批量入口以及桌面和移动排版。订单关联用例检查客户联动、编号金额和日期倒序，物流用例单独检查永久档案统计与响应式布局。
