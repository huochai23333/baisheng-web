...
- `wholesale/`：批发业务模块，包含批发订单、分批结汇记录、1688 订单认领、物流、客户管理、批发推荐树、客户推荐佣金和业务员提成；管理员额外可查看批发人员管理，不复用 `admin-orders/`；批发 VIP 使用独立客户权益模型，不复用旅游 VIP 表。
...
- 批发客户、批发订单、物流、客户推荐和 1688 采购单认领的写入权限以数据库为准：管理员可以管理全部批发数据；业务员只能新增、编辑和删除自己创建或分配给自己的批发客户，不能替其他业务员建客户或转派客户；财务可以查看批发客户并维护自己可处理范围内的订单、物流费用和推荐关系，不负责客户档案写入。物流状态核对记录对管理员、财务和业务员全部可见，客户只查看自己绑定批发客户名下的物流状态。
