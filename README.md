...
| 角色            | 默认入口          | 当前模块                                                                                                                                                                                                                                                                               |
| --------------- | ----------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `administrator` | `/admin/home`     | 全局首页、审核中心、账号管理、公告管理、公司费用、反馈管理、汇率设置；旅游业务下的订单、客户管理、推荐树、团队、人员管理、VIP管理、操作记录、佣金、任务和业务设置；批发业务下的批发订单、库存订单、结汇发布、订单认领、物流、客户管理、人员管理、VIP管理、推荐树、佣金、提成和业务设置 |
| `salesman`      | `/salesman/home`  | 全局首页；固定仅可见批发业务，显示批发订单、库存订单、结汇发布、订单认领、物流、客户管理、VIP管理、推荐树、佣金和提成                                                                                                                                                                  |
...
- 管理员、业务员可以创建、修改、一次登记全额实付、作废和维护 Order List；不支持部分实付。客户和财务对订单字段及附件只读。
- 客户只能在自己的待支付订单选择固定200美元、一单库存订单金额的50%、所有订单金额的5%中的一个或多个档位。后两个名称不触发自动百分比计算，最终美元金额由审核人填写。
- 管理员、业务员和财务一次处理同一订单的全部待审档位。批准金额按审批当天汇率锁定到订单币种，信贷抵消与剩余实付必须正好覆盖购买金额，随后订单立即付清。
...
- 库存订单可以按关键词、客户、业务员、支付状态、币种和录入日期筛选；信贷记录可以按关键词、客户、信贷档位、信贷状态、到期情况和申请日期筛选。客户页面隐藏没有意义的客户、业务员筛选，筛选无结果时可以一键清空。
- 页面模块按 `queries / actions hook / filter view-model / filters / order section / credit section / dialogs / display` 拆分；桌面使用紧凑表格，移动端使用信息卡片和自动换行的筛选区。
