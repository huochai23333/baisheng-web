...
- `wholesale/`：批发业务模块，包含批发订单、Order List 附件、分批结汇记录、结汇发布、1688 订单认领、物流、客户管理、批发推荐树、客户推荐佣金和业务员提成；订单类型、页面查询、范围裁剪、附件 mutation、附件权限 hook、弹窗和表格各自使用同层模块，管理员额外可查看批发人员管理，不复用 `admin-orders/`；批发 VIP 使用独立客户权益模型，不复用旅游 VIP 表。
...
- 批发客户、批发订单、结汇发布、物流、客户推荐和 1688 采购单认领的写入权限以数据库为准：管理员可以管理全部批发数据；业务员只能新增、编辑和删除自己创建或分配给自己的批发客户，不能替其他业务员建客户或转派客户；财务可以查看批发客户并发布或取消结汇收款，不负责客户档案写入。物流状态核对记录对管理员、财务和业务员全部可见；客户的物流状态和物流费用记录都必须明确关联到自己的一笔批发订单，只匹配客户但未关联订单的记录不会显示。
...
- 每笔批发订单可以保存最多 10 个 Order List 附件，只接受 CSV、XLS 和 XLSX；单文件不超过 20 MB，单笔订单附件合计不超过 100 MB。管理员、财务可以管理可见订单附件，业务员只能管理自己负责订单的附件，其他内部只读角色和订单所属客户可以下载但不能上传或删除。附件保存在私有 `wholesale-order-lists` bucket，下载时临时生成签名地址。
