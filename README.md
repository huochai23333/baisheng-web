...
- `LOGISTICS_SOURCE_STATUS_API_URL` 和 `LOGISTICS_SOURCE_STATUS_API_TOKEN` 只配置为 Supabase Edge Function secrets，用于批发物流状态每日核对，不暴露到浏览器端；物流同步只调用受控接口，不直接读取外部项目数据库。
...
- 服务端权限判断必须先使用 Supabase Auth 验证用户身份，再从 `get_current_app_access_context` 读取数据库角色和状态；数据库上下文读取失败时不使用 Auth 元数据或客户端会话兜底放行。登录后的客户端会话只用于界面状态和会话同步，不能作为服务端放行依据。
...
- 新批发业务保留在同一个后台壳内，但不复用旅游订单结构；批发客户、订单、订单修改申请、订单修改记录、结汇发布、1688 采购单、物流费用记录、物流状态核对、客户推荐佣金、业务员提成、推荐关系和 VIP 权益分别走 `wholesale_customers`、`wholesale_orders`、`wholesale_order_edit_requests`、`wholesale_order_change_logs`、`wholesale_settlement_releases`、`wholesale_1688_orders`、`wholesale_logistics_orders`、`wholesale_logistics_statuses`、`wholesale_commissions`、`wholesale_referrals` 和批发 VIP 相关表等独立模型。
...
- 批发客户管理中，管理员查看全部批发客户，业务员只查看自己创建或分配给自己的批发客户，以及与这些客户相关的账号资料。批发客户以客户唯一标识名称作为业务上的唯一客户名，可以先不绑定系统账号，由业务员登记联系方式、其他名称和客户来源；后续可以在客户详情里通过“新增名称”继续补充更多昵称、店铺名或对账名称。管理员和业务员可以在客户详情里编辑客户唯一名称、其他名称、联系方式、来源、关联业务员和备注；未产生批发订单的客户可以删除，已有批发订单的客户不能删除。客户完成注册后，管理员或负责该客户的业务员可以在客户详情里选择客户注册账号进行关联合并，系统不会新建第二个批发客户记录，而是把注册账号绑定到原客户并开通批发业务可见权限，客户登录后可以查看同一客户名下的订单、物流、推荐关系、佣金和 VIP 权益。批发推荐树以“公司”作为唯一主分支，所有已创建批发客户都会出现在公司下方；已有客户之间的推荐关系继续作为客户下级展示，展示层的公司节点不写入推荐关系表，也不参与推荐佣金计算。
- 批发客户、批发订单、结汇发布、物流、客户推荐和 1688 采购单认领的写入权限以数据库为准：管理员可以管理全部批发数据；业务员只能新增、编辑和删除自己创建或分配给自己的批发客户，不能替其他业务员建客户或转派客户；财务可以查看批发客户并发布或取消结汇收款，不负责客户档案写入。物流状态核对记录对管理员、财务和业务员全部可见，客户只查看自己绑定批发客户名下的物流状态。
...
- 批发订单状态分为“未结汇”“部分结汇”和“已结汇”；新建订单默认未结汇，下单时间与订单号生成时间一致。关联业务员、建单人或管理员可以在订单列表中登记多笔结汇记录，每次填写本次结汇金额和结汇日期；日期可以使用当天，也可以指定其他日期，系统按所选日期的对应币种人民币汇率计算本次人民币金额。所选日期没有汇率时，需要先到汇率设置中补充，结汇弹窗不手动填写汇率。
...
- 结汇发布是订单结汇之外的协作入口。财务和管理员在批发业务的“结汇发布”中发布收到的客户结汇款，可以选择现有批发客户，也可以先填写临时客户名；每条记录包含金额、币种、收款日期和备注。所有业务员都能看到待认领记录，但认领时只能匹配到自己可管理的一笔批发订单。发布记录金额必须一次性放入一笔订单，不能拆到多笔订单；币种必须与订单客户支付币种一致，选择了现有客户时只能匹配同一客户订单。财务手填临时客户名时，认领弹窗不按客户 ID 限制，并支持按客户名、订单号、订单金额和下单日期筛选可匹配订单。认领成功后，系统按收款日期写入订单结汇记录，并把发布记录标记为已匹配。财务和管理员可以取消待认领记录；业务员不能发布或取消结汇收款。
...
- 一笔批发订单可以关联多条 1688 采购订单，也可以关联多条物流状态或物流费用记录。批发订单默认按本月范围读取首批 20 条，使用下单时间和订单 ID 组成稳定降序游标；页面显示“已显示 X / 总数”，点击“继续加载”追加下一批且不能重复或遗漏。筛选由数据库对当前账号可见的完整订单范围执行，覆盖订单号、客户、业务员、快递、备注、关联 1688 单号、来源物流订单号、国际/目的地物流号和物流状态；筛选变化会清空旧批次并从第一页重新加载。新建、修改、申请修改、审核申请或登记结汇后也会刷新当前筛选的第一页。当前筛选范围可以生成业务口径的 AI 订单评估，完整范围的订单数量和金额由数据库汇总，页面只取有限订单作为示例。
- 批发订单桌面端保留完整横向表格，订单号和操作固定在左侧，结汇记录在单元格内滚动；只渲染已经加载的批次。390px 等移动宽度不显示压缩后的完整表格，改为订单卡片：首屏先看到搜索和筛选，卡片展示订单号、客户、状态、支付金额、结汇进度、业务员和下单时间，点击后在带焦点锁的详情弹窗中分组查看费用、利润、结汇、关联采购/物流和备注，并保留修改、申请修改与登记结汇入口。移动端“本月概览”默认折叠，页面本身不能横向溢出。1688 采购订单和物流管理各自的桌面表格仍按对应板块规则展示。
- 订单核心查询失败时页面必须显示加载失败和重试入口；结汇、采购、物流或修改记录等关联查询失败时显示对应局部提醒，不能用“暂无记录”掩盖失败。订单页只查询本板块所需的客户、账号、汇率、修改规则和当前订单批次，不再读取佣金、推荐或完整采购物流数据。
- 1688 采购订单可以通过页面上传从 1688 导出的订单表格，也可以通过登录后的 `/api/wholesale/1688-orders` 接口接收，进入订单认领列表；接口每次最多接收 5 MiB、500 行，同一用户 10 分钟最多接收 5 次且同时只能执行 1 次，响应结束后释放并发名额。页面上传支持 1688 导出的 Excel 订单表格并保留 CSV 文件接收能力。订单认领页分为“已认领 / 待分类 / 认领大厅”。API 接收的 1688 订单如果能根据客户或批发订单信息确定归属，会自动进入已认领；页面上传从订单认领页按钮打开弹窗完成，不直接铺在页面顶部，订单行会先按收货人名字辅助匹配批发客户，匹配到客户的进入待分类，业务员确认关联批发订单后进入已认领；没有辅助匹配到客户的进入认领大厅，由业务员手动认领，认领按钮显示在固定订单号列下方，操作列保留移出等管理动作。业务员只能认领并归属自己可管理的客户和批发订单，认领后列表显示认领者用户名，原始采购订单内容进入系统后不允许随意修改，误上传数据只能由管理员移出认领列表。
...
- 本地 Supabase 测试数据会生成 100 条当前月份的批发订单，订单列表默认当月筛选时应能直接看到足够的订单样本。
