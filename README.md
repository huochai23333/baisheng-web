...
- `app/theme.css` 与 `components/ui/`：全站唯一的浅色视觉入口，基准是暖白工作台。页面画布使用 `#faf9f7`，导航结构使用 `#f4f3f1`，筛选、表格和弹窗使用 `#fbfaf8`；具体色值只存在于主题 CSS，业务 TSX 使用表面、内容、边框、品牌和状态等语义令牌。页头和重要面板保留轻阴影，普通数据区与嵌套筛选只使用边界，菜单和弹窗等浮层使用强阴影。普通面板移动端为 24px 圆角、桌面端为 28px 圆角，不使用背景模糊；模糊只留给顶栏、移动菜单等覆盖结构，以及认证页经过约束的唯一玻璃外壳。项目当前不提供深色模式。
...
- 每条结汇发布记录包含金额、币种、收款日期和备注。所有业务员都能看到待认领记录，并可以匹配到任一业务员负责的批发订单。发布记录金额必须一次性放入一笔订单，不能拆到多笔订单；币种必须与订单客户支付币种一致，选择了现有客户时只能匹配同一客户订单。认领弹窗先确认客户再选择订单：发布时已经选择客户的记录会预填并锁定该客户，临时客户名记录只列出拥有可匹配订单的客户；订单继续按币种、未结清状态和剩余可结汇金额筛选。认领成功后，系统按收款日期写入订单结汇记录，并把发布记录标记为已匹配。财务和管理员可以取消待认领记录；业务员不能发布或取消结汇收款，即使直接请求助手确认接口也会被服务端和数据库拒绝。
...
- 批发浏览器回归会拦截真实写请求，验证订单、附件、客户别名、账号合并、1688 导入、单条与批量认领组、物流店铺归属、推荐、结汇发布和结汇匹配失败时表单保持打开、字段不丢失、反馈可见；1688 用例还覆盖收货人和采购日期筛选、当前结果全选、多订单选择、认领组调整与撤销、刷新后持久化、四类角色权限以及桌面和移动排版。订单关联用例检查客户联动、编号金额、日期倒序、详情展示和关联单号搜索，物流用例单独检查永久档案统计与响应式布局。
