...
| `operator`      | `/business-unavailable` | 当前角色原有业务已停用，登录后进入停用说明页 |
...
- `business-settings/`：当前只组装批发订单修改规则、批发订单业务员提成和批发客户推荐佣金区块。
...
- 业务入口不由管理员页面实时开关。管理员、财务、业务员及拥有批发权限的客户进入批发业务；只有旅游权限的账号保留登录会话并进入停用说明页。
...
- 外侧设置保留汇率设置；批发业务设置维护批发订单修改规则、批发订单业务员提成和批发客户推荐佣金。旅游业务设置不再提供入口。
...
- `tests/e2e/legacy-tourism-disabled.spec.ts` 覆盖旅游专属账号登录、旧旅游地址、未知地址、导航与邀请入口、伪造 API 请求和 390px 排版；批发工作流继续由原有批发专项用例覆盖。
