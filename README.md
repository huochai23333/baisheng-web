...
- `wholesale/`：批发业务模块，包含批发订单、Order List 附件、分批结汇记录、结汇发布、1688 订单认领、物流、客户管理、批发推荐树、客户推荐佣金和业务员提成；订单类型、页面查询、范围裁剪、附件 mutation、附件权限 hook、弹窗和表格各自使用同层模块，角色能力集中在 `lib/wholesale-role-permissions.ts`，管理员额外可查看批发人员管理，不复用 `admin-orders/`；批发 VIP 使用独立客户权益模型，不复用旅游 VIP 表。物流协调组件只组装创建/关联/解除弹窗与两张列表，分页查询、筛选 hook、每日核对表、历史费用表和 mutation 分别放在同层模块及 `lib/wholesale-logistics-page.ts`；关联订单的标签、客户过滤和日期排序由独立共享工具统一处理。`useWholesaleActions` 只组合共享 action runner 与订单、附件、1688、物流/推荐/佣金、客户 action 模块，不直接承载具体请求。
- 批发写操作统一返回 `Promise<boolean>`：数据库确认成功返回 `true`，失败返回 `false` 并保留弹窗、表单输入和已解析文件。共享 runner 统一维护 `pendingKey` 与页面反馈；一般批发页面成功后使用服务端刷新，订单页和物流页明确禁用服务端刷新，只刷新当前筛选的受影响列表，禁止同一次操作执行两套刷新。物流关联使用记录级 pending key，一条记录保存时不会锁住整张表。失败反馈固定显示在弹窗遮罩上方，页面文案只使用用户能直接理解的说明。
...
- 批发客户管理中，管理员和业务员都查看全部批发客户、全部批发业务员资料，以及已经绑定批发客户的注册账号资料。批发客户以客户唯一标识名称作为业务上的唯一客户名，可以先不绑定系统账号，由业务员登记联系方式、其他名称和客户来源；后续可以在客户详情里通过“新增名称”继续补充更多昵称、店铺名或对账名称。管理员和任一业务员都可以编辑客户唯一名称、其他名称、联系方式、来源、关联业务员和备注，也可以在有效批发业务员之间转派；未产生批发订单的客户可以删除，已有批发订单的客户不能删除。客户完成注册后，管理员或任一业务员可以在客户详情里选择符合条件的注册账号进行关联合并，系统不会新建第二个批发客户记录，而是把注册账号绑定到原客户并开通批发业务可见权限，客户登录后可以查看同一客户名下的订单、物流、推荐关系、佣金和 VIP 权益。批发推荐树以“公司”作为唯一主分支，所有已创建批发客户都会出现在公司下方；已有客户之间的推荐关系继续作为客户下级展示，展示层的公司节点不写入推荐关系表，也不参与推荐佣金计算。
- 批发客户、批发订单、结汇发布、物流、客户推荐和 1688 采购单认领的写入权限以数据库为准：管理员可以管理全部批发数据；业务员之间共享日常业务范围，可以新增、编辑、删除和转派彼此的客户与订单，也可以协作处理 VIP、客户推荐、1688 归属、物流、订单结汇和 Order List 附件。业务员不会因此获得订单修改审批、业务设置、删除 1688 导入记录或其他管理员专属能力，个人业务提成仍只展示给受益业务员本人。财务继续沿用原有工作范围，可以查看批发客户并发布或取消结汇收款，不负责跨业务员转派客户档案。物流状态核对记录对管理员、财务和业务员全部可见；可修改范围仍由每条记录和目标订单的数据库权限决定。客户的物流状态和物流费用记录都必须明确关联到自己的一笔批发订单，只匹配客户但未关联订单的记录不会显示，客户页面也不显示创建、关联或解除入口。
...
- 所有人工关联批发订单的弹窗统一先选客户，再选择该客户当前账号可见的订单；更换客户会清空订单，不自动选中唯一订单或第一条订单。订单选项统一显示“订单编号 · 原币订单金额”，按下单时间倒序、订单 ID 倒序稳定排列。调整现有关联时预填当前客户和订单，失败时保留选择。一笔批发订单可以关联多条 1688 采购订单、多条物流状态或物流费用记录和多个 Order List 附件。批发订单默认按本月范围读取首批 20 条，使用下单时间和订单 ID 组成稳定降序游标；页面显示“已显示 X / 总数”，点击“继续加载”追加下一批且不能重复或遗漏。筛选由数据库对当前账号可见的完整订单范围执行，覆盖订单号、客户、业务员、快递、备注、关联 1688 单号、来源物流订单号、国际/目的地物流号和物流状态；客户搜索不使用收款平台。筛选变化会清空旧批次并从第一页重新加载。新建、修改、申请修改、审核申请、登记结汇或上传附件仅在成功后刷新当前筛选的第一页；失败时不刷新、不清空列表，也不关闭当前表单。删除附件只局部更新附件列表并保持管理弹窗打开，方便连续处理多份文件。内部账号可以为当前筛选范围生成业务口径的 AI 订单评估，客户不显示该入口。
- 批发订单桌面端保留完整横向表格，订单号和操作固定在左侧，结汇记录和 Order List 文件在单元格内滚动；只渲染已经加载的批次。390px 等移动宽度不显示压缩后的完整表格，改为订单卡片：首屏先看到搜索和筛选，卡片展示订单号、客户、状态、支付金额、结汇进度、业务员和下单时间，点击后在带焦点锁的详情弹窗中分组查看费用、利润、结汇、关联采购/物流、Order List 附件和备注，并保留当前角色有权执行的操作。移动端“本月概览”默认折叠，页面本身不能横向溢出。1688 采购订单和物流管理各自的桌面表格仍按对应板块规则展示。
...
- 物流模块以“物流号、客户名、当前状态”为核心。系统每天从第二物流库自动导入物流号、客户名和最新状态，并把客户名精确匹配到现有批发客户；未匹配到客户的记录仍对管理员、财务和业务员可见，方便后台处理。出现已送达、已签收、退回、丢失、取消、无法投递或异常终止后停止继续核对。页面标题区只保留“创建物流记录”按钮，点击后在弹窗填写物流号、来源客户名、归属客户和可选批发订单；订单选择遵循统一的客户联动、编号金额展示和日期倒序规则，失败时保留全部输入。物流页面只读取订单编号、原币金额、币种、客户和下单时间，不读取成本或利润字段。
- 每日核对与历史物流费用是两套独立列表，各自默认读取 50 条、显示当前已加载数量和完整筛选总数，并使用排序字段与记录 ID 组成稳定游标继续加载。每日核对按“未结束优先、最近更新优先”排列，可按物流号/客户/订单/当前状态关键词、客户、关联情况和物流状态筛选；历史费用按最近更新时间排列，可按国际/目的地运单号、来源订单、客户名称及别名、批发订单号、物流状态和货代关键词、客户及关联情况筛选。筛选和搜索都针对当前账号可见的全部数据库记录，不再限制最近 500 条，搜索输入使用延迟值减少连续请求。
- 自动同步记录和历史费用记录都可以关联、调整或解除批发订单。关联弹窗先选客户，再选该客户订单；数据库始终用订单所属客户校正记录的客户 ID，所以后续自动同步不会覆盖人工订单关联。解除只清空订单关联，保留校正后的客户与来源客户名/来源订单快照，并在确认弹窗明确提示解除后客户将看不到该记录。操作成功只刷新受影响列表，失败时弹窗与选择保持不变。
...
- 批发物流状态每日核对由 Supabase `wholesale-logistics-status-sync` Edge Function 执行；该函数只调用物流库的 `logistics-status-lookup` 接口，先自动导入物流号和客户名，再更新物流状态。主页面只读取本项目 Supabase 里的物流状态镜像表，通过数据库筛选和每页 50 条的稳定游标分页控制大量记录的加载量。
...
- `tests/e2e/wholesale-logistics.spec.ts` 覆盖创建弹窗联动、两类全量筛选、关联/改绑/解除、真实 50 条继续加载、管理员/业务员/财务/客户权限，以及 1440px 和 390px 的横向溢出与文字挤压。
...
- 批发浏览器回归会拦截真实写请求，验证订单、附件、客户别名、账号合并、1688 导入与归属、物流、推荐、结汇发布和结汇匹配失败时表单保持打开、字段不丢失、反馈可见；关联订单用例还会检查客户联动、订单编号与原币金额、日期倒序，以及 1440px 与 390px 宽度下的横向溢出和文字挤压。
