...
- `wholesale/`：批发业务模块，包含批发订单、Order List 附件、分批结汇记录、结汇发布、1688 订单认领、物流、客户管理、批发推荐树、客户推荐佣金和业务员提成；订单类型、页面查询、范围裁剪、附件 mutation、附件权限 hook、弹窗和表格各自使用同层模块，角色能力集中在 `lib/wholesale-role-permissions.ts`，管理员额外可查看批发人员管理，不复用 `admin-orders/`；批发 VIP 使用独立客户权益模型，不复用旅游 VIP 表。`useWholesaleActions` 只组合共享 action runner 与订单、附件、1688、物流/推荐/佣金、客户 action 模块，不直接承载具体请求。
- 批发写操作统一返回 `Promise<boolean>`：数据库确认成功返回 `true`，失败返回 `false` 并保留弹窗、表单输入和已解析文件。共享 runner 统一维护 `pendingKey` 与页面反馈；一般批发页面成功后使用服务端刷新，订单页 action 明确禁用服务端刷新，只在成功后刷新当前筛选第一页，禁止同一次操作执行两套刷新。失败反馈固定显示在弹窗遮罩上方，页面文案只使用用户能直接理解的说明。
...
- 批发客户、批发订单、结汇发布、物流、客户推荐和 1688 采购单认领的写入权限以数据库为准：管理员可以管理全部批发数据；业务员之间共享日常业务范围，可以新增、编辑、删除和转派彼此的客户与订单，也可以协作处理 VIP、客户推荐、1688 归属、物流、订单结汇和 Order List 附件。业务员不会因此获得订单修改审批、业务设置、删除 1688 导入记录或其他管理员专属能力，个人业务提成仍只展示给受益业务员本人。财务继续沿用原有工作范围，可以查看批发客户并发布或取消结汇收款，不负责跨业务员转派客户档案。物流状态核对记录对管理员、财务和业务员全部可见；客户的物流状态和物流费用记录都必须明确关联到自己的一笔批发订单，只匹配客户但未关联订单的记录不会显示。
...
- `tests/e2e/wholesale-mutation-failures.spec.ts` 会拦截真实浏览器发出的写请求，验证订单、附件、客户别名、账号合并、1688 导入与归属、物流、推荐和结汇发布失败时表单保持打开、字段不丢失、反馈可见，并在 1440px 与 390px 宽度检查横向溢出。
