...
- `wholesale/`：批发业务模块，包含批发订单、Order List 附件、分批结汇记录、结汇发布、1688 订单认领、物流、客户管理、批发推荐树、客户推荐佣金和业务员提成；订单类型、页面查询、范围裁剪、附件 mutation、附件权限 hook、弹窗和表格各自使用同层模块，角色能力集中在 `lib/wholesale-role-permissions.ts`，管理员额外可查看批发人员管理，不复用 `admin-orders/`；批发 VIP 使用独立客户权益模型，不复用旅游 VIP 表。
