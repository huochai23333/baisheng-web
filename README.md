...
| `salesman` | `/salesman/home` | 全局首页；固定仅可见批发业务，显示批发订单、订单认领、物流、客户管理、人员管理、VIP管理、推荐树、佣金和提成 |
...
- `wholesale/`：批发业务模块，包含批发订单、1688 订单认领、物流、客户管理、人员管理、批发推荐树、客户推荐佣金和业务员提成，不复用 `admin-orders/`；批发 VIP 使用独立客户权益模型，不复用旅游 VIP 表。
...
- 各业务下客户管理与人员管理分开。旅游客户进入旅游业务的客户管理，旅游人员管理只保留旅游业务人员；批发客户进入批发业务的客户管理，批发人员管理只保留可承接批发客户和订单的业务员账号。
...
- 批发业务客户管理可以新增、搜索、筛选、查看批发客户，并与注册账号关联合并；批发业务人员管理只查看批发承接人账号。VIP 开通、续费和有效期调整进入批发 VIP 管理。
