...
- 越权访问不展示中间错误页，也不改写到其他角色工作台；系统会先清理当前会话，再进入安全登录页，不把 `0.0.0.0` 这类监听地址显示给用户。
...
- `app/forbidden.tsx`：服务端权限拦截后的统一处理入口，直接进入退出登录流程并回到登录页。
...
- 一笔批发订单可以关联多条 1688 采购订单，也可以关联多条物流状态或物流费用记录；批发订单列表直接展示这两类关联记录的摘要，默认显示当月下单的订单，日期筛选框同步显示当月第一天到最后一天；搜索订单时同时匹配关联采购单、物流单号和物流状态，并支持按下单日期选择开始和结束日期查看特定时间段的订单；选择好当前范围后，可以生成业务口径的 AI 订单评估，评估内容使用普通中文展示，覆盖订单数量、结汇状态、客户支付金额、采购成本、各类费用、毛利表现、风险点和下一步建议。
- 批发订单、1688 采购订单和物流运单列表采用横向表格直接展示完整信息，不再通过点击订单打开详情弹窗；订单类列表的订单号固定在左侧，物流列表的运单号固定在左侧，方便横向滑动时仍能对齐当前行。批发订单固定列只保留订单编号和操作按钮，不在订单编号下重复展示结汇标识，并使用紧凑操作区避免订单之间出现过大的纵向空白；结汇记录在单元格内部滚动，不能把整行撑出明显大空白；关联 1688 列只显示 1688 订单号，点击后打开 1688 订单详情弹窗；关联物流列只显示物流订单号或物流号，点击后打开物流订单详情弹窗。批发表格长列表在表格区域内滚动，表头固定在列表顶部，查看下方订单时仍能看到订单编号、客户、业务员等字段含义；移动端继续保留横向滑动，不压缩列宽导致文字竖排或按钮挤压。
- 1688 采购订单可以通过页面上传从 1688 导出的订单表格，也可以通过登录后的 `/api/wholesale/1688-orders` 接口接收，进入订单认领列表；页面上传支持 1688 导出的 Excel 订单表格并保留 CSV 文件接收能力。订单认领页分为“已认领 / 待分类 / 认领大厅”。API 接收的 1688 订单如果能根据客户或批发订单信息确定归属，会自动进入已认领；页面上传从订单认领页按钮打开弹窗完成，不直接铺在页面顶部，订单行会先按收货人名字辅助匹配批发客户，匹配到客户的进入待分类，业务员确认关联批发订单后进入已认领；没有辅助匹配到客户的进入认领大厅，由业务员手动认领。业务员只能认领并归属自己可管理的客户和批发订单，认领后列表显示认领者用户名，原始采购订单内容进入系统后不允许随意修改，误上传数据只能由管理员移出认领列表。
