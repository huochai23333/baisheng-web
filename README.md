...
| 角色 | 默认入口 | 当前模块 |
| --- | --- | --- |
| `administrator` | `/admin/home` | 全局首页、账号管理、公告管理、反馈管理、汇率设置；旅游业务下的订单、推荐树、团队、旅游人员、VIP管理、操作记录、佣金、任务、审核和业务设置；批发业务下的批发订单、订单认领、物流、人员管理、VIP管理、推荐树、佣金、提成和业务设置 |
| `salesman` | `/salesman/home` | 全局首页；固定仅可见批发业务，显示批发订单、订单认领、物流、人员管理、VIP管理、推荐树、佣金、提成和业务设置 |
| `promoter` | `/promoter/home` | 全局首页；固定仅可见旅游业务，显示旅游订单、人员、VIP管理、推荐树、团队、佣金和任务 |
...
- `/admin/tourism/settings` 是旅游业务设置，维护旅游订单服务费、服务价格、服务折扣和旅游相关佣金。
...
- `tourism-people/`：旅游业务人员管理，按“旅游客户 / 地推账户”查看旅游侧人员资料，账号身份和状态调整回到全局账号管理，VIP 状态和申请不在这里处理。
- `admin-orders/`：旅游业务订单，配合订单查询、表单、详情、权限和错误映射模块；不承载新批发订单结构。
- `admin-system-settings/`：管理员全局汇率设置，只维护自动获取币种、手动补充汇率和历史记录。
- `business-settings/`：业务内设置页，旅游业务注册服务费、服务价格、服务折扣和旅游佣金区块，批发业务注册批发订单业务员提成和批发客户推荐佣金区块。
...
- `wholesale/`：批发业务模块，包含批发订单、1688 订单认领、物流、人员管理、批发推荐树、客户推荐佣金和业务员提成，不复用 `admin-orders/`；批发 VIP 使用独立客户权益模型，不复用旅游 VIP 表。
...
- 新批发业务保留在同一个后台壳内，但不复用旅游订单结构；批发客户、订单、1688 采购单、物流、客户推荐佣金、业务员提成、推荐关系和 VIP 权益分别走 `wholesale_customers`、`wholesale_orders`、`wholesale_1688_orders`、`wholesale_logistics_orders`、`wholesale_commissions`、`wholesale_referrals` 和批发 VIP 相关表等独立模型。
...
- 外侧设置只保留汇率设置，维护自动获取币种、手动补充汇率和历史记录；旅游业务设置放在旅游业务侧栏内，维护旅游订单服务费、服务价格、服务折扣和旅游相关佣金；批发业务设置放在批发业务侧栏内，维护批发订单业务员提成和批发客户推荐佣金。
...
- 旅游业务人员管理按“旅游客户 / 地推账户”两个页签查看旅游侧人员资料；列表只做搜索、状态筛选和详情查看，不承载账号身份调整，也不承载 VIP 申请或审核。
- 批发业务人员管理按“批发客户 / 业务员账户”两个页签查看批发侧人员资料；批发客户可以新增、搜索、筛选、查看详情并与注册账号关联合并，业务员账户只作为批发承接人列表查看，VIP 开通、续费和有效期调整进入批发 VIP 管理。
